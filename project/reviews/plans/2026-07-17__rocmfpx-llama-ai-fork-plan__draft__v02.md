...
status: revised-for-independent-review
created: 2026-07-17
...
Imported Wiki scripts and fixtures are preserved candidate evidence, not trusted executable dependencies. Before any imported conformance or validation code runs:

1. record package/file hashes, provenance, and licenses;
2. statically review scripts, fixtures, subprocess/network behavior, paths, dependencies, and generated outputs;
3. define a bounded isolated execution environment with no credentials, production services, writable model/cache roots, or unapproved network access;
4. validate deterministic behavior against synthetic fixtures and retain raw results;
5. promote only the accepted subset into an approved canonical test-assets location with a review record.

Until this gate closes, the cross-fork conformance package supplies requirements/fixture ideas only. **OPEN-TEST-01** owns the intake decision and accepted asset manifest.
...
| L08 | persistent provider/retention | read-only then read-write canary, quotas, reserve, LRU, optional prefetch; depends on L04-L07 | restart/quota/rollback gates; `persistent-cache-v1-canary` |
...
- **OPEN-TEST-01:** review and approve the exact imported conformance/validation assets and isolated execution contract before running them.
...
12. bounded canary and retention (L08);
13. optional L09-L14 capabilities one lane at a time;
