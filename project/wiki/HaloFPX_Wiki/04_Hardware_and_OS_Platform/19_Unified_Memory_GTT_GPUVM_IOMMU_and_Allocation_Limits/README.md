...
- **[MEASURED]** During the 2026-08-12 nimo-2 incident, ordinary accounting still reported about 14 GiB available while the production worker held `114041696 kB` of `gpu_active` HMM pages. Four global OOM invocations killed user-session processes and then the production worker [S19-L04]. `MemAvailable`, free RAM, swap, and conventional RSS are therefore insufficient admission predicates under high HMM ownership.
