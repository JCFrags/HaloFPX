# Research Agent Prompt — 80: Fault Injection: Cable Pulls, Restarts, OOM, Disk-Full, and Corruption
...
Research and build the wiki section **80: Fault Injection: Cable Pulls, Restarts, OOM, Disk-Full, and Corruption**.

**Required coverage:** Create a systematic failure matrix covering each cable, both cables, worker and coordinator crashes, process kill, GPU reset, OOM, allocation failure, stale versions, malformed messages, checksum failures, torn cache commits, disk-full, read-only filesystem, NVMe errors, USB4 retraining, and recovery assertions.
...
```text
11_Verification_and_Performance/80_Fault_Injection_Cable_Pulls_Restarts_OOM_Disk_Full_and_Corruption/
```
