# Research Agent Prompt — 81: CI Matrix, Release Gates, Reproducibility, and Performance Regression Policy
...
Research and build the wiki section **81: CI Matrix, Release Gates, Reproducibility, and Performance Regression Policy**.

**Required coverage:** Define fast per-commit tests, nightly hardware tests, model smoke tests, kernel/backend matrices, format compatibility, cache migrations, distributed checks, quality gates, security scans, artifact retention, benchmark variance policy, regression thresholds, release candidate qualification, and signed build provenance.
...
```text
11_Verification_and_Performance/81_CI_Matrix_Release_Gates_Reproducibility_and_Performance_Regression_Policy/
```
