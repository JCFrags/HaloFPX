# Research Agent Prompt — 71: Security, Trust Boundaries, Permissions, Local Network, and Secrets
...
Research and build the wiki section **71: Security, Trust Boundaries, Permissions, Local Network, and Secrets**.

**Required coverage:** Develop a threat model for two physically connected hosts and local clients. Cover API authentication, Unix permissions, link trust, optional transport authentication or encryption, cache privacy, multi-user isolation, malicious model files, path traversal, RPC attack surface, kernel privileges, supply chain, secrets, logs, and secure defaults.
...
```text
10_Product_Server_and_Operations/71_Security_Trust_Boundaries_Permissions_Local_Network_and_Secrets/
```
