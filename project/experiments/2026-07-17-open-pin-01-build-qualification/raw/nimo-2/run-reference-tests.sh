...
log=$root/nimo-2-reference-tests.log
overall=0
run_cmd() {
  name="$1"; shift
  echo "===== $name ====="
...
for lane in control candidate; do
  ...
  run_cmd "$lane-rocmfp4-quant-regression" env ROOT="$src" BUILD_DIR="$src/build-ref-rocmfp4-quant" "$src/scripts/check-rocmfp4-quant-regression.sh"
  run_cmd "$lane-test-turboquant" env LD_LIBRARY_PATH="$build/bin" "$build/bin/test-turboquant"
  run_cmd "$lane-test-backend-help" env LD_LIBRARY_PATH="$build/bin" "$build/bin/test-backend-ops" --help
  run_cmd "$lane-test-backend-list" env LD_LIBRARY_PATH="$build/bin" "$build/bin/test-backend-ops" --list
  echo "===== $lane artifact hashes ====="
  find "$build/bin" -maxdepth 1 -type f -executable -print0 | sort -z | xargs -0 sha256sum
  echo "===== $lane linked libraries ====="
...
printf '%s\n' "$overall" > "$root/nimo-2-reference-tests.rc"
exit "$overall"