# Research Agent Prompt — 12: Codebase Architecture and Module Map
...
Research and build the wiki section **12: Codebase Architecture and Module Map**.

**Required coverage:** Create a source-level map of llama.cpp and the two forks: model loading, GGUF, ggml backends, scheduler, server, RPC, KV cache, state serialization, speculative decoding, quantization, HIP, Vulkan, tests, scripts, and packaging. Identify key files, call paths, extension points, ownership boundaries, and coupling risks.
...
```text
03_Repository_and_Engineering/12_Codebase_Architecture_and_Module_Map/
```
