# Research Agent Prompt — 74: Single-Node HIP and Vulkan Baseline Matrix
...
Research and build the wiki section **74: Single-Node HIP and Vulkan Baseline Matrix**.

**Required coverage:** Create the baseline plan across both machines, model families, quants, contexts, batch sizes, KV types, FlashAttention, MTP settings, power modes, and backends. Require prompt and decode breakdowns, TTFT, p50/p95/p99 inter-token latency, memory use, quality checks, thermals, and reproducible command lines.
...
```text
11_Verification_and_Performance/74_Single_Node_HIP_and_Vulkan_Baseline_Matrix/
```
