# Research Agent Prompt — 48: Distributed Correctness, Determinism, Fault Recovery, and Degraded Mode
...
Research and build the wiki section **48: Distributed Correctness, Determinism, Fault Recovery, and Degraded Mode**.

**Required coverage:** Define numerical and state correctness across ranks, deterministic test modes, sequence ownership, collective ordering, cancellation, timeout, partial failure, worker restart, cable loss, one-link operation, model mismatch, cache mismatch, resynchronization, request replay, and rules for falling back to a safe single-node or replicated mode.
...
```text
07_Distributed_Runtime/48_Distributed_Correctness_Determinism_Fault_Recovery_and_Degraded_Mode/
```
