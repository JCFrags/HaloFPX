# Research Agent Prompt — 39: Coordinator, Rank Worker, Session, and Persistent-Graph Architecture
...
Research and build the wiki section **39: Coordinator, Rank Worker, Session, and Persistent-Graph Architecture**.

**Required coverage:** Design the coordinator and rank-worker processes, responsibilities, startup handshake, model-shard validation, session ownership, tokenizer and sampler placement, graph lifetime, buffer lifetime, command flow, health state, and shutdown. Define which data and decisions are global, rank-local, session-local, or request-local.
...
```text
07_Distributed_Runtime/39_Coordinator_Rank_Worker_Session_and_Persistent_Graph_Architecture/
```
