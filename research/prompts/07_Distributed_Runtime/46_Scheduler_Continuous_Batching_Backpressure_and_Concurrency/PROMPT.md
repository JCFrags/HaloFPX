# Research Agent Prompt — 46: Scheduler, Continuous Batching, Backpressure, and Concurrency
...
Research and build the wiki section **46: Scheduler, Continuous Batching, Backpressure, and Concurrency**.

**Required coverage:** Design request admission, slot allocation, sequence batching, prompt/decode scheduling, pipeline overlap, cache-aware placement, user limits, fairness, backpressure, cancellation, streaming, priority, and overload behavior across one or two ranks. Define scheduling metrics and invariants that preserve correctness under mixed workloads.
...
```text
07_Distributed_Runtime/46_Scheduler_Continuous_Batching_Backpressure_and_Concurrency/
```
