# Research Agent Prompt — 61: Attention KV, Recurrent, MTP, Speculative, Sampling, and RNG State
...
Research and build the wiki section **61: Attention KV, Recurrent, MTP, Speculative, Sampling, and RNG State**.

**Required coverage:** Enumerate all state required for exact continuation across dense attention, MLA, sliding window, Mamba/SSM hybrids, native MTP, external speculation, samplers, grammar constraints, RNG, logits processors, and server sequence metadata. Define independent versioned streams and atomic restore requirements.
...
```text
09_HaloKV_Persistent_Cache/61_Attention_KV_Recurrent_MTP_Speculative_Sampling_and_RNG_State/
```
