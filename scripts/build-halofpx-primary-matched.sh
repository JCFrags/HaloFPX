...
cmake -S "${source_root}" -B "${build_root}" -G Ninja \
    ...
    -DGGML_HIP_NO_VMM=ON \
    -DGGML_HIP_ROCMFPX_QKV_Q8_REUSE="${GGML_HIP_ROCMFPX_QKV_Q8_REUSE:-OFF}" \
    -DGGML_HIP_ROCMFPX_MMVQ_QKV_Q8_REUSE="${GGML_HIP_ROCMFPX_MMVQ_QKV_Q8_REUSE:-OFF}" \
...
grep -E '^(CMAKE_BUILD_TYPE|CMAKE_C_COMPILER:|CMAKE_CXX_COMPILER:|CMAKE_HIP_COMPILER:|CMAKE_HIP_ARCHITECTURES:|GPU_TARGETS:|GGML_HIP:|GGML_HIP_FORCE_MMQ:|GGML_HIP_NO_VMM:|GGML_HIP_ROCMFPX_QKV_Q8_REUSE:|GGML_HIP_ROCMFPX_MMVQ_QKV_Q8_REUSE:|GGML_HIP_QUANT_KV_FATTN_TILE:|GGML_RPC:|GGML_VULKAN:|GGML_VULKAN_FA_Q8_0_PREDEQUANT:|LLAMA_BUILD_SERVER:|LLAMA_BUILD_TESTS:|LLAMA_BUILD_WEBUI:)' \
