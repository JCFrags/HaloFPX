...
template <ggml_type type_KV, typename T>
static __device__ __forceinline__ void halofpx_fattn_tile_dequant_pair(
        const char * const __restrict__ row, T * const __restrict__ dst, const int i0) {
    static_assert(type_KV == GGML_TYPE_Q8_0 || type_KV == GGML_TYPE_Q4_0, "unsupported quantized KV type");
    if constexpr (type_KV == GGML_TYPE_Q8_0) {
        dequantize_V_q8_0<T, 2>(row, dst, i0);
    } else {
        dequantize_V_q4_0<T, 2>(row, dst, i0);
    }
}

template<ggml_type type_KV, int warp_size, int nwarps, int I, int J, int J_padding, bool oob_check>
static __device__ __forceinline__ void halofpx_fattn_tile_load_quantized(
        const char * const __restrict__ KV, half2 * const __restrict__ tile_KV,
        const int64_t stride_KV, const int i_sup, const int j0) {
    static_assert(J % 2 == 0, "bad J");
    constexpr int pairs_per_row = J/2;
    constexpr int nthreads = warp_size*nwarps;
    const int tid = threadIdx.y*warp_size + threadIdx.x;

    for (int index = tid; index < I*pairs_per_row; index += nthreads) {
        const int i = index / pairs_per_row;
        const int j = index - i*pairs_per_row;
        half2 value = {0.0f, 0.0f};
        if (!oob_check || i < i_sup) {
            halofpx_fattn_tile_dequant_pair<type_KV>(KV + int64_t(i)*stride_KV, (half *) &value, j0 + 2*j);
        }
        tile_KV[i*(pairs_per_row + J_padding) + j] = value;
    }
}
...
static __device__ __forceinline__ void halofpx_fattn_tile_load_quantized(
    ...
    for (int index = tid; index < I*pairs_per_row; index += nthreads) {
        ...
        if (!oob_check || i < i_sup) {
            halofpx_fattn_tile_dequant_pair<type_KV>(KV + int64_t(i)*stride_KV, (float *) &value, j0 + 2*j);
        }
        ((float2 *) (tile_KV + i*(J + J_padding)))[j] = value;
    }
}
#endif
...
#if defined(GGML_USE_HIP) && defined(GGML_HIP_QUANT_KV_FATTN_TILE)
    if constexpr (type_K == GGML_TYPE_Q8_0 || type_K == GGML_TYPE_Q4_0) {
        halofpx_fattn_tile_load_quantized<type_K, warp_size, nwarps, nbatch_fa, nbatch_K, cpy_ne, oob_check>
            (K + int64_t(k_VKQ_0)*stride_K, KV_tmp, stride_K, k_VKQ_sup, k_KQ_0);
    } else
#endif
    {
        const half2 * K_h2 = (const half2 *) K;
        const int stride_K2 = stride_K / sizeof(half2);
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < DKQ - nbatch_K_last; k_KQ_0 += nbatch_K) {
        flash_attn_tile_iter_KQ<warp_size, nwarps, ncols1, ncols2, DKQ, nbatch_fa, nbatch_K, use_logit_softcap, oob_check,
            T_vec_dot, type_K>(Q_tmp, K, KV_tmp, stride_K, k_VKQ_0, k_VKQ_sup, k_KQ_0, KQ_acc);
    }
    if (nbatch_K_last > 0) {
...
#if defined(GGML_USE_HIP) && defined(GGML_HIP_QUANT_KV_FATTN_TILE)
        if constexpr (type_V == GGML_TYPE_Q8_0 || type_V == GGML_TYPE_Q4_0) {
            halofpx_fattn_tile_load_quantized<type_V, warp_size, nwarps, nbatch_V, DV, 0, oob_check>
                (V + int64_t(k_VKQ_0 + k0)*stride_V, KV_tmp, stride_V, k_VKQ_sup - k0, 0);
        } else
#endif
        {
            const half2 * V_h2 = (const half2 *) V;
            const int stride_V2 = stride_V / sizeof(half2);
    ...
    if (ncols2 == 1) {
        ...
        while (k_VKQ_0 < k_VKQ_max - nbatch_fa) {
            ...
            flash_attn_tile_iter<warp_size, nwarps, ncols1, ncols2, DKQ, DV, nbatch_fa, nbatch_K, use_logit_softcap, oob_check,
                type_K, type_V>(Q_tmp, K_data, V_data, maskh, ne01, logit_softcap, slope, KQ, KV_tmp,
...
#if defined(GGML_USE_HIP) && defined(GGML_HIP_QUANT_KV_FATTN_TILE)
#define DECL_FATTN_TILE_QUANT_CASE(DKQ, DV, type_KV)                       \
    template void ggml_cuda_flash_attn_ext_tile_case                       \
    <DKQ, DV, type_KV, type_KV>(ggml_backend_cuda_context & ctx, ggml_tensor * dst) \

extern DECL_FATTN_TILE_QUANT_CASE(128, 128, GGML_TYPE_Q8_0);
extern DECL_FATTN_TILE_QUANT_CASE(128, 128, GGML_TYPE_Q4_0);
extern DECL_FATTN_TILE_QUANT_CASE(256, 256, GGML_TYPE_Q8_0);
extern DECL_FATTN_TILE_QUANT_CASE(256, 256, GGML_TYPE_Q4_0);
#endif
