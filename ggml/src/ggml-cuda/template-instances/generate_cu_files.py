...
DECL_FATTN_TILE_CASE({head_size_kq}, {head_size_v});{quantized_cases}
"""

SOURCE_FATTN_TILE_QUANTIZED_CASES = """

#if defined(GGML_USE_HIP) && defined(GGML_HIP_QUANT_KV_FATTN_TILE)
DECL_FATTN_TILE_QUANT_CASE({head_size_kq}, {head_size_v}, GGML_TYPE_Q8_0);
DECL_FATTN_TILE_QUANT_CASE({head_size_kq}, {head_size_v}, GGML_TYPE_Q4_0);
#endif
"""
...
AUTOGENERATED_HEADER = "// This file has been autogenerated by generate_cu_files.py, do not edit manually."

for filename in glob("*.cu"):
    with open(filename, "r") as source:
        generator_owned = source.readline().rstrip("\r\n") == AUTOGENERATED_HEADER
    if generator_owned:
        os.remove(filename)
...
for head_size_kq in HEAD_SIZES_KQ:
    ...
    quantized_cases = ""
    if head_size_kq in (128, 256) and head_size_kq == head_size_v:
        quantized_cases = SOURCE_FATTN_TILE_QUANTIZED_CASES.format(
            head_size_kq=head_size_kq, head_size_v=head_size_v).rstrip()
    with open(f"fattn-tile-instance-dkq{head_size_kq}-dv{head_size_v}.cu", "w") as f:
        f.write(SOURCE_FATTN_TILE.format(
            head_size_kq=head_size_kq,
            head_size_v=head_size_v,
            quantized_cases=quantized_cases))
