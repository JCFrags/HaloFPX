...
The `nimo-1/` and `nimo-2/` directories retain the configure/build logs,
...
remote capture checksums. Absolute `/tmp` paths in raw records are the paths on
the named target at capture time.
