...
bool snapshot_message(const context_store_bootstrap_authority_config & config,
        const context_store_format_digest & anchor_secret_digest, bounded_buffer & message) noexcept {
...
context_store_bootstrap_authority::context_store_bootstrap_authority(
    ...
    if (!valid_key(anchor.disposition, anchor.key_id, anchor.master_key) ||
        ...
        anchor.generation == 0 || admin.generation == 0 || config.policy_epoch == 0 ||
        ...
        same_secret(anchor.master_key, admin.master_key)) return;
    ...
    const bool ok = context_store_sha256_bounded(anchor_key_.data(), anchor_key_size_,
        ...
        snapshot_message(owned_config, anchor_secret_digest, message) &&
...
context_store_bootstrap_authority::~context_store_bootstrap_authority() noexcept {
    ...
    anchor_key_generation_ = admin_key_generation_ = 0;
    anchor_key_size_ = admin_key_size_ = 0;
    valid_ = false;
}
...
context_store_bootstrap_result context_store_bootstrap_authority::plan(
    ...
    if (!nonzero_bytes(request.attempt_id) || !nonzero_bytes(request.selected_manifest_digest) ||
        request.object_count == 0 || request.object_count > context_store_manifest_max_objects) {
        result.status = context_store_bootstrap_status::invalid_request;
        return result;
    }

    auto body = bootstrap_body_;
    body.selected_manifest_digest = request.selected_manifest_digest;
    ...
    const bool authorized = anchor_digest != nullptr &&
        plan_message(snapshot_commitment_, request.attempt_id, request.object_count,
            request.selected_manifest_digest, *anchor_digest, message) &&
        context_store_hmac_sha256(admin_key_.data(), admin_key_size_, message.bytes.data(), message.size,
            authorization);
    wipe(message.bytes.data(), message.bytes.size());
    wipe(encoded.data(), encoded.size());
    ...
    result.plan_.object_count_ = request.object_count;
    result.plan_.selected_manifest_digest_ = request.selected_manifest_digest;
