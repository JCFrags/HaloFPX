...
bool context_store_registry_lab_admit_quarantine_v1(const context_store_registry_lab_quarantine_value_v1&v,const context_store_registry_lab_credential&c,const context_store_registry_lab_quarantine_evidence_v1&x,context_store_registry_lab_quarantine_witness&o)noexcept{if((!v.attributable&&(nonzero(v.attempt_id)||v.slot||nonzero(v.operation_commitment)))||(!v.has_previous_record&&nonzero(v.previous_record_digest))||(!v.has_head&&nonzero(v.head_digest))||(x.transition&&x.standalone_predecessor_head_value))return false;context_store_registry_lab_expectation e{};if(x.transition){if(!map_prepare(*x.transition,e))return false;}else if(x.standalone_predecessor_head_value){if(v.attributable||v.has_previous_record||!v.has_head||v.phase||x.standalone_predecessor_head_value->selection!=context_store_registry_lab_head_selection_v1::predecessor||x.successor_head||x.successor_head_size||x.prepare||x.prepare_size||!same_scope_value(v.scope,x.standalone_predecessor_head_value->scope)||x.standalone_predecessor_head_value->expected_head_digest!=v.head_digest||!map_head(*x.standalone_predecessor_head_value,e))return false;}else map_scope(v.scope,e);if(x.prepare&&!hash_domain(content_domains[size_t(context_store_registry_lab_kind::prepare)],x.prepare,x.prepare_size,e.prepare_digest))return false;if(!x.standalone_predecessor_head_value){if(x.successor_head)e.successor_head_digest=v.head_digest;else if(x.predecessor_head&&v.has_head)e.initial_head_digest=v.head_digest;}e.quarantine_event_id=v.event_id;e.attempt_id=v.attempt_id;e.quarantine_previous_record_digest=v.previous_record_digest;e.quarantine_head_digest=v.head_digest;e.operation_commitment=v.operation_commitment;e.slot=v.slot;e.quarantine_reason=v.reason;e.quarantine_phase=v.phase;e.quarantine_attributable=v.attributable;e.quarantine_has_previous_record=v.has_previous_record;e.quarantine_has_head=v.has_head;context_store_registry_lab_lifecycle_witness::evidence z{x.predecessor_head,x.predecessor_head_size,x.successor_head,x.successor_head_size,x.prepare,x.prepare_size};bool ok=context_store_registry_lab_lifecycle_witness::admit_quarantine(c,e,z,o.storage_);if(ok)o.value_=v;return ok;}
...
bool context_store_registry_lab_registry_envelope_digest_v1(
    ...
    context_store_format_digest computed {};
    if (!registry_digest(data, size, computed)) {
        return false;
    }
    output = computed;
